...
def fetch_price_history(code):
    """J-Quantsから取得可能な全期間の株価。実際の終値(C)を優先する。"""
    ...
    for r in rows:
        ...
        # 価格推移・IPO価格との比較では実際の終値(C)を優先。
        # 分割等の調整値は別用途なのでここでは採用しない。
...
YFINANCE_LOOKBACK_DAYS = int(os.environ.get("YFINANCE_LOOKBACK_DAYS", "84"))  # 約12週間

def to_yfinance_symbol(jquants_code):
    """J-Quantsの銘柄コードをYahoo Finance/yfinance用の東証シンボルへ変換。"""
    code = str(jquants_code).strip()
    if len(code) == 5 and code[-1] == "0":
        code = code[:4]
    return code + ".T"

def fetch_yfinance_history(code, start_date, end_date):
    """J-Quantsで不足する直近12週間だけをyfinanceから取得する。"""
    if yf is None:
        print("yfinanceがインストールされていません。", file=sys.stderr)
        return []
    ...
    try:
        ...
        if df is None or df.empty:
            return []
        # yfinanceのバージョンによってMultiIndexになる場合に対応
        close_col = "Close"
        if hasattr(df.columns, "nlevels") and df.columns.nlevels > 1:
            close_series = df["Close"].iloc[:, 0] if "Close" in df.columns.get_level_values(0) else None
        else:
            close_series = df[close_col] if close_col in df.columns else None
...
def merge_price_histories(jq_history, yf_history):
    """同一日を重複させず、J-Quantsを基本としてyfinanceで不足日だけ補完する。"""
    by_date = {row["date"]: row for row in jq_history if row.get("date") and row.get("price") is not None}
    # yfinanceはJ-Quantsに存在しない日だけ追加。重複日はJ-Quantsを優先。
    for row in yf_history:
        if row.get("date") and row.get("price") is not None and row["date"] not in by_date:
...
def main():
    ...
    for code, first_seen in final_targets.items():
        ...
        # J-Quantsで取得できない直近12週間だけをyfinanceで補完。
        # J-Quantsの最終日より後の日付を中心に取得し、重複日はJ-Quantsを優先する。
        yf_end = today + datetime.timedelta(days=1)
        yf_start = today - datetime.timedelta(days=YFINANCE_LOOKBACK_DAYS)
        if as_of:
            try:
                jq_last_date = datetime.date.fromisoformat(as_of)
                supplement_start = max(yf_start, jq_last_date + datetime.timedelta(days=1))
            except ValueError:
                supplement_start = yf_start
        else:
            supplement_start = yf_start
        yf_history = fetch_yfinance_history(
            code, supplement_start.isoformat(), yf_end.isoformat()
        )
        price_history = merge_price_histories(price_history, yf_history)
        as_of = price_history[-1]["date"] if price_history else as_of

        time.sleep(REQUEST_INTERVAL_SEC)
        financials = fetch_financials(code)

        current_price = price_history[-1]["price"] if price_history else None
        price_source = "yfinance" if yf_history and price_history and price_history[-1]["date"] in {r["date"] for r in yf_history} else "j-quants"
        price_as_of = as_of
        ...
        output.append({
            ...
            "dataSource": "J-Quants API（過去データ）＋ yfinance（J-Quantsで不足する直近12週間）",
